...
import { User, Lock, UploadCloud, DownloadCloud } from "lucide-react"; // Imported new icons for backup
...
// --- Content Component: Backup Tab (NEW) ---

const BackupTabContent: React.FC = () => (
  <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-md space-y-6">
    <h2 className="text-lg font-semibold text-gray-800">
      Data Backup & Restore
    </h2>
    <p className="text-sm text-gray-500">
      Manage backups of your application data. We recommend taking regular
      backups.
    </p>
    ...
    <div className="flex items-center justify-between border-b border-gray-100 pb-4">
      <div className="flex items-center space-x-3">
        <UploadCloud className="text-pink-500" size={24} />
        <p className="text-base text-gray-700">Create New Backup</p>
      </div>
      <button className="bg-green-500 hover:bg-green-600 text-white font-medium py-2 px-4 rounded-lg transition duration-150 text-sm">
        Backup Now
      </button>
    </div>
    ...
    <div className="flex items-center justify-between">
      <div className="flex items-center space-x-3">
        ...
        <div>
          <p className="text-base text-gray-700">Restore Data</p>
          <p className="text-xs text-gray-400">Last backup: October 10, 2025</p>
        </div>
      </div>
      <button className="border border-blue-500 text-blue-500 hover:bg-blue-50 font-medium py-2 px-4 rounded-lg transition duration-150 text-sm">
        Restore
      </button>
    </div>
  </div>
          ...
          {accountTabs.map((tab) => (
            <button
              ...
              className={`
                            ...
                            `}
            >
              {/* Icon is defined but not used in the button yet. Keeping the logic simple for now. */}
              {tab.name}
            </button>
          ))}
        </div>
